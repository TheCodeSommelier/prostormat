...
CanonicalRails.setup do |config|
  ...
  config.protocol = Rails.env.development? ? 'http://' : 'https://'

  # This is the main host, not just the TLD, omit slashes and protocol. If you have more than one, pick the one you want to rank in search results.

  config.host = Rails.env.development? ? 'localhost' : 'www.prostormat.cz'
  config.port = '3000' if Rails.env.development?
  ...
  config.collection_actions = %i[index show about_us faq_contact_us overload landing_page]

  # Parameter spamming can cause index dilution by creating seemingly different URLs with identical or near-identical content.
  # Unless allowed, these parameters will be omitted

  # config.allowed_parameters = ['pages=1']
