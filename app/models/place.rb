...
class Place < ApplicationRecord
  ...
  validates :place_name, :street, :city, :house_number, :postal_code, :max_capacity, :short_description,
            :long_description, presence: true

  validates :max_capacity, numericality: { only_integer: true, greater_than: 10 }
  validates :postal_code, format: { with: /\A\d{3}\s\d{2}\z/ }

  validates :place_name, :street, format: { with: /\A[\w\s]+\z/ } # Allows for underscores, letters, numbers and spaces
  validates :short_description, length: { in: 30..100 }

  validates :long_description, length: { minimum: 120 }
